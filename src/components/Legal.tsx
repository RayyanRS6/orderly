...
const documents: Record<LegalKind, { title: string; intro: string; sections: [string, string][] }> =
  {
    privacy: {
      ...
      sections: [
        ...
        [
          'Service providers',
          'Cloudflare hosts the website. Supabase provides authentication, the database and backend functions. Meta delivers WhatsApp messages. The business’s chosen AI provider (OpenAI, Google Gemini or Anthropic) receives message content and relevant business context to interpret requests. Google receives order data when Sheets is connected. If staff enable email alerts and the mail service is configured, Resend receives the verified staff email and a generic operational notice without customer details. Providers may process data outside Pakistan under their own terms. The business must review its provider’s tier and data-use settings before enabling AI. Orderly does not sell personal information or use customer chats to train its own models.',
        ],
        [
          'Retention and security',
          'There is no automatic customer-data expiry enabled by default. Owners can preview and enable automatic cleanup in Security & privacy for records inactive for 30-3650 days. A changed policy has a 24-hour grace period. Cleanup runs in bounded batches and protects unfinished orders, human handoffs and outstanding jobs; busy workspaces are skipped. Businesses should choose a period appropriate to their recordkeeping obligations. Authorized owners can export and delete a customer’s conversations and orders from the active database after pausing processing. Anonymous usage accounting and administrative audit events are retained for billing and security. Copies in Google Sheets, WhatsApp, AI providers, downloaded exports and backups have separate retention and must be handled separately. Deleted data may remain in provider backups until their expiry. Access is restricted by business membership, API credentials are encrypted at rest, and traffic uses HTTPS.',
        ],
        [
          'Your choices',
