...
- Local app remains in demo mode, preserving existing demo data. Live founder user creation is pending in Supabase Authentication; user must enter and submit their own login password. Intended founder email: `waytogalaxy999@gmail.com`. Platform-admin membership has not yet been granted.
- GitHub repository, Vercel deployment, Google Sheets service account and spreadsheet remain pending. Vercel account currently uses Hobby; checked-in every-minute recovery cron needs Pro before deployment as configured. No paid upgrade purchased.
